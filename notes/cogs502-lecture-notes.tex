...
\chead{COGS 502 - Symbols and Prog.}
...
\item Three building blocks: \uterm{symbols}, \uterm{numbers}, \uterm{addresses} of memory locations called \uterm{cells}.
	...
	\end{uenumi}
\item A fourth type of entity is a \uterm{cons}, a concatenation of two addresses, which is stored in a cell. 
	\begin{uenumi}
	\item Therefore, an address, by virtue of uniquely identifying a cell, can point to a symbol, a number, or a cons.\footnote{\ldots or a procedure.}
...
%\item The list is the basic data structure in \Verb+LISP+ and in many other programming languages. Once you have your data in the form of a list, then you can do various transformations and/or checks on your data. 
...
Evaluating \Verb+(total *grades*)+ would give an error. The reason this is unsuccessful is that the \Verb=+= operator works on numbers, but we provide a list as an argument. Using \Verb+FUNCALL+ would not help as well. The construct to use in such situations is \Verb+APPLY+. It is like \Verb+FUNCALL+ with the only difference that the arguments are provided in a list.\footnote{Actually, the rule for \Verb+APPLY+ is more flexible than this, only the last argument to it needs to be a list, but we will ignore this possibility for now.} 
...
% 
% There is a very useful construct to store values that are to be used more than once. The construct is \Verb+LET+ and here is its syntax:
